...
class Lead(models.Model):
    ...
    tag_ids = fields.Many2many(
        ...
        help="Classify and analyze your lead/opportunity categories like: Training, Service", store=True,
        index='trigram')
    ...
    website = fields.Char('Website', help="Website of the contact", compute="_compute_website", readonly=False,
                          store=True)
    ...
    duplicate_lead_ids = fields.Many2many("crm.lead", compute="_compute_potential_lead_duplicates",
                                          string="Potential Duplicate Lead", context={"active_test": False})
    duplicate_lead_count = fields.Integer(compute="_compute_potential_lead_duplicates",
    ...
    # CUSTOM FIELDS
    create_date = fields.Datetime('Created On', store=True, readonly=False)
    create_date_tmp = fields.Datetime('Created On Temp', store=True, readonly=False)
    id_tmp = fields.Char('ID Temp', store=True)
    x_address = fields.Text('Address', store=True)
    x_company_linkedin = fields.Char('Company LinkedIn', store=True)
    x_remote_role = fields.Char('Remote Role', store=True)
    x_personal_linkedin = fields.Char('Personal LinkedIn', store=True)
    x_direct_number_1 = fields.Char('Direct Number 1', store=True)
    x_direct_number_2 = fields.Char('Direct Number 2', store=True)
    x_company_phone_2 = fields.Char('Company Phone 2', store=True)
    x_neverbounce_result = fields.Char('Neverbounce Result', store=True)
    x_time_zone = fields.Char('Time Zone', store=True)

    # HISTORY TAB
    x_created_date = fields.Datetime('Created Date', store=True)
    x_owner_assigned_date = fields.Datetime('Owner Assigned Date', store=True)
    x_last_activity_date = fields.Datetime('Last Activity Date', store=True)
    x_last_modified_date = fields.Datetime('Last Modified Date', store=True)
    x_next_activity_date = fields.Datetime('Next Activity Date', store=True)
    x_close_date = fields.Datetime('Close Date', store=True)
    x_last_contacted = fields.Datetime('Last Contacted', store=True)
    x_date_of_last_meeting_booked_in_meetings_tool = fields.Datetime('Date of Last Meeting Booking in Meetings Tool',
                                                                     store=True)
    x_campaign_of_last_booking_in_meetings_tool = fields.Datetime('Campaign of Last Booking in Meetings Tool',
                                                                  store=True)
    x_source_of_last_booking_in_meetings_tool = fields.Datetime('Source of Last Booking in Meetings Tool', store=True)
    x_closed_won_reason = fields.Text('Closed Won Reason', store=True)
    x_closed_lost_reason = fields.Text('Closed Lost Reason', store=True)
    x_original_source_data_1 = fields.Char('Original Source Data 1', store=True)
    x_original_source_data_2 = fields.Char('Original Source Data 2', store=True)
    x_original_source_type = fields.Selection([('ORGANIC_SEARCH', 'Organic Search'),
                                               ...
                                               ('PAID_SOCIAL', 'Paid Social'), ], string='Original Source Type',
                                              store=True)
    # SEAMLESS AI FILTERS
    x_sa_seniorities = fields.Selection([('C_LEVEL', 'C-Level'),
                                         ('VP', 'VP'),
                                         ('DIRECTOR', 'Director'),
                                         ('MANAGER', 'Manager'),
                                         ('SENIOR', 'SENIOR'),
                                         ('OTHER', 'Other'), ], 'Seniorities', store=True)
    x_sa_departments = fields.Selection([('SALES', 'Sales'),
                                         ('MARKETING', 'Marketing'),
                                         ...
                                         ('FINANCE', 'Finance'),
                                         ('SUPPORT', 'Support'),
                                         ('OTHER', 'Other')], 'Departments', store=True)
    x_sa_industries = fields.Char('Industries', store=True)
    x_sa_company_city = fields.Char('Company City', store=True)
    x_sa_company_state = fields.Char('Company State', store=True)
    x_sa_locations = fields.Text('Location', store=True)
    x_sa_employee_sizes = fields.Selection([('SELF_EMPLOYED', 'Self-Employed'),
                                            ('2_10', '2-10 Employees'),
                                            ...
                                            ('10001_up', '10001+ Employees')], 'Employee Sizes', store=True, index=True)
    x_sa_estimated_revenue = fields.Selection([('0-100K', '$0-$100K'),
                                               ('100K-1M', '$100K-$1M'),
                                               ('1M-5M', '$1M-$5M'),
                                               ('5M-20M', '$5M-$20M'),
                                               ('20M-50M', '$20M-$50M'),
                                               ...
                                               ('100M-500M', '$100M-$500M'),
                                               ('500M-1B', '$500M-$1B'),
                                               ('1B_up', '$1B+')], 'Estimated Revenue', store=True, index=True)

    # OPPORTUNITY EMAIL BLAST
    x_fi_batch_number = fields.Char('FI Batch Number', store=True)
    x_fi_sending_email_address = fields.Char('FI Sending Email Address', store=True)
    x_fi_sent_email_1 = fields.Boolean('FI Sent Email #1', store=True)
    x_fi_sent_email_2 = fields.Boolean('FI Sent Email #2', store=True)
    x_fi_sent_email_3 = fields.Boolean('FI Sent Email #3', store=True)
    x_fi_sent_email_4 = fields.Boolean('FI Sent Email #4', store=True)
    x_fi_sent_email_5 = fields.Boolean('FI Sent Email #5', store=True)
    x_fi_sent_email_6 = fields.Boolean('FI Sent Email #6', store=True)
    x_fi_opened_email_1 = fields.Boolean('FI Opened Email #1', store=True)
    x_fi_opened_email_2 = fields.Boolean('FI Opened Email #2', store=True)
    x_fi_opened_email_3 = fields.Boolean('FI Opened Email #3', store=True)
    x_fi_opened_email_4 = fields.Boolean('FI Opened Email #4', store=True)
    x_fi_opened_email_5 = fields.Boolean('FI Opened Email #5', store=True)
    x_fi_opened_email_6 = fields.Boolean('FI Opened Email #6', store=True)

    # Leads Email Blast
    x_leads_batch_number = fields.Char('Leads Batch Number', store=True, invisible=False)
    x_leads_sending_email_address = fields.Char('Leads Sending Email Address', store=True)
    x_leads_sent_email_1_2022 = fields.Boolean('Leads Sent Email #1 - 2022', store=True)
    x_leads_sent_email_1_2023 = fields.Boolean('Leads Sent Email #1 - 2023', store=True)
    x_leads_opened_email_1_2022 = fields.Boolean('Leads Opened Email #1 - 2022', store=True)
    x_leads_opened_email_1_2023 = fields.Boolean('Leads Opened Email #1 - 2023', store=True)



    #three numbers , phone validation

    x_start_time = fields.Datetime(default=fields.Datetime.now)
    x_running_time = fields.Char(compute='_compute_running_time')
    x_has_more_than_three_numbers = fields.Boolean('More than 3 numbers?', store=True)

    # CUSTOM FUNCTIONS

    @api.depends('x_start_time')
    def _compute_running_time(self):
        for record in self:
            record.x_running_time = str(fields.Datetime.now() - record.x_start_time)

            numbers = [record.x_direct_number_1, record.phone, record.x_direct_number_2, record.x_company_phone_2]
            record.x_has_more_than_three_numbers = len([num for num in numbers if num]) >= 3

    #end of phone validation

    # CUSTOM FUNCTIONS
    def add_opt_out_email(self):
        tag_model = self.env['crm.tag']
        opt_out_tag = tag_model.search([('name', '=', 'opt-out email')], limit=1)
        if opt_out_tag:
            self.write({'tag_ids': [(4, opt_out_tag.id)]})

    def button_click_email(self):
        self.add_opt_out_email()

    def add_opt_out_call(self):
        tag_model = self.env['crm.tag']
        opt_out_tag = tag_model.search([('name', '=', 'opt-out call')], limit=1)
        if opt_out_tag:
            self.write({'tag_ids': [(4, opt_out_tag.id)]})

    def button_click_call(self):
        self.add_opt_out_call()
    ...
    def _compute_recurring_revenue_monthly_prorated(self):
        for lead in self:
            lead.recurring_revenue_monthly_prorated = (lead.recurring_revenue_monthly or 0.0) * (
    ...
    def _onchange_phone_validation(self):
        if self.phone:
            self.phone = self.phone_get_sanitized_number(number_fname='phone',
                                                         force_format='INTERNATIONAL') or self.phone
    ...
    def search(self, args, offset=0, limit=None, order=None, count=False):
        ...
        my_lead_ids_ordered = sorted(search_res.ids, key=lambda lead_id: my_lead_mapping[lead_id],
    ...
    def action_set_won(self):
        ...
        for lead in self:
            ...
            if not stage_id:
                stage_id = next((stage for stage in reversed(won_stages) if stage.sequence <= lead.stage_id.sequence),
                                won_stages)
    ...
    def _get_opportunity_meeting_view_parameters(self):
        ...
        """
        self.ensure_one()
        meeting_results = self.env["calendar.event"].search_read([('opportunity_id', '=', self.id)],
                                                                 ['start', 'stop', 'allday'])
        ...
        if relevant_meeting_count == 1:
            ...
            meetings_in_same_week = latest_stop_dt <= datetime(next_week_start_date.year, next_week_start_date.month,
                                                               next_week_start_date.day, 0, 0, 0)
    ...
    def get_empty_list_help(self, help):
        ...
        if alias_record and alias_record.alias_domain and alias_record.alias_name:
            ...
            sub_title = _(
                'Use the top left <i>Create</i> button, or send an email to %s to test the email gateway.') % (
                            email_link)
    ...
    def log_meeting(self, meeting_subject, meeting_date, duration):
        ...
        message = _("Meeting scheduled at '%s'<br> Subject: %s <br> Duration: %s") % (
    ...
    def _merge_opportunity(self, user_id=False, team_id=False, auto_unlink=True, max_length=5):
        ...
        if max_length and len(self.ids) > max_length and not self.env.is_superuser():
            raise UserError(
                _("To prevent data loss, Leads and Opportunities can only be merged by groups of %(max_length)s.",
        ...
        if merged_data.get('team_id'):
            team_stage_ids = self.env['crm.stage'].search(
                ['|', ('team_id', '=', merged_data['team_id']), ('team_id', '=', False)], order='sequence, id')
    ...
    def _merge_dependences_history(self, opportunities):
        ...
        for opportunity in opportunities:
            for message in opportunity.message_ids:
                if message.subject:
                    subject = _("From %(source_name)s : %(source_subject)s", source_name=opportunity.name,
    ...
    def _message_get_suggested_recipients(self):
        ...
        try:
            for lead in self:
                ...
                lang_code = lead.lang_code if lead.lang_code and self.env['res.lang']._lang_get(
                    lead.lang_code) else None
                if lead.partner_id:
    ...
    def _pls_get_naive_bayes_probabilities(self, batch_mode=False):
        ...
        frequencies = self.env['crm.lead.scoring.frequency'].search([('variable', 'in', list(leads_fields))],
                                                                    order="team_id asc, id")
        ...
        result = dict((team_id, dict((field, dict(won_total=0, lost_total=0)) for field in leads_fields)) for team_id in
                      frequency_team_ids)
    ...
    def _pls_increment_frequencies(self, from_state=None, to_state=None):
        ...
        """
        new_frequencies_by_team, existing_frequencies_by_team = self._pls_prepare_update_frequency_table(
    ...
    def _pls_prepare_update_frequency_table(self, rebuild=False, target_state=False):
        ...
        if rebuild:  # rebuild is ok
            ...
            team_ids = self.env['crm.team'].with_context(active_test=False).search([]).ids + [
                0]  # If team_id is unset, consider it as team 0
        else:  # increment
    ...
    def _pls_prepare_frequencies(self, lead_values, leads_pls_fields, target_state=None):
        ...
        for values in lead_values:
            ...
            if 'tag_id' in values:
                frequencies = self._pls_increment_frequency_dict(frequencies, 'tag_id', values['tag_id'], won_count,
                                                                 lost_count)
                continue
                ...
                if value or field in ('email_state', 'phone_state'):
                    if field == 'stage_id':
                        ...
                        for stage_id in stages_to_increment:
                            frequencies = self._pls_increment_frequency_dict(frequencies, field, stage_id, won_count,
                                                                             lost_count)
                    else:
                        frequencies = self._pls_increment_frequency_dict(frequencies, field, value, won_count,
                                                                         lost_count)
    ...
    def _pls_get_lead_pls_values(self, domain=[]):
        ...
        if domain:
            ...
            from_clause, where_clause, where_params = self.env['crm.lead'].with_context(active_test=False)._where_calc(
                domain).get_sql()
            ...
            for lead in lead_results:
                lead_values = []
                for field in pls_fields + [
                    'probability']:  # add probability as used in _pls_prepare_frequencies (needed in rebuild mode)
                    value = lead[field]
